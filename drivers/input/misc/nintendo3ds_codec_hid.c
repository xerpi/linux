...
static void nintendo3ds_codec_hid_poll(struct input_dev *input)
{
	struct nintendo3ds_codec_hid *codec_hid = input_get_drvdata(input);
...
static int nintendo3ds_codec_hid_probe(struct spi_device *spi)
{
	...
	if (!codec_hid) {
		dev_err(dev, "%s: Can't allocate CODEC HID struct, error %d\n",
			__func__, err);
		err = -ENOMEM;
	...
	if (!input_dev) {
		dev_err(dev, "%s: Can't allocate input device, error %d\n",
			__func__, err);
		err = -ENOMEM;
	...
	input_set_drvdata(input_dev, codec_hid);
	...
	codec_hid->spi = spi;
	codec_hid->input_dev = input_dev;
	vkb_init(&codec_hid->vkb);
	spi_set_drvdata(spi, codec_hid);

	spi_codec_hid_initialize(spi);
	
	err = input_setup_polling(input_dev, nintendo3ds_codec_hid_poll);
	if (err) {
		dev_err(dev, "nintendo3ds_codec_hid.c: could not setup polling\n");
		goto err_free_dev;
	}
	...
	if (err) {
		dev_err(dev, "nintendo3ds_codec_hid.c: could not register input device\n");
		goto err_free_dev;
	}
